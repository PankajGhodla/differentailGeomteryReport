...
We will now discuss about the \textbf{geodesic curvature}. Intuitively, it measures how quickly a surface is bending away from the plane \( \widetilde{T_pS}\) at point \(p\), where \( \widetilde{T_pS}\) = p + \text{span}\{ v, N(p)\}. To see this, we decompose the acceleration, i.e. the second derivative, of \( \gamma\) as follows:
\begin{align*}
...
We will now define Gaussian curvature with respect to the local coordinates u\text{sin}g the following proposition:
\begin{proposition}
    Let \( \begin{pmatrix}
        w_{11} & w_{21} \\
        w_{21} & w_{22}
